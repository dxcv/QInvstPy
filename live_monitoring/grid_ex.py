...
# 合约代码准备
api = TqApi(TqSim(), backtest=TqBacktest(start_dt=dt.date(2018,9,10), end_dt=dt.date(2018,11,16)))

# 网格计划参数:
symbol = "CZCE.SR811"  # 合约代码
start_price = 4944.63  # 起始价位
...
async def price_watcher(open_price, close_price, volume):
    ...
    update_chan = api.register_update_notify(quote) # 当 quote 有更新时会发送通知到 update_chan 上
    while True:
