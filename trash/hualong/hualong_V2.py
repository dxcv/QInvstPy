...
def wave_rule(dict_results, Y=0.0025, Z=0.015):
    ...
    if (Y < (Wave_up[1] / Wave_up[0] - 1) < Z) and \
            Wave_up[0] < MA.loc[ls_up[0]] and \
            MA.loc[ls_up[1]] < Wave_up[1] and \
            (np.abs(pricet - MA[-1]) < 0.6):
        signal = 1
        stop_loss = Wave_up[0]
        target_price = pricet + h_up
    elif (-Z < (Wave_down[1] / Wave_down[0] - 1) < -Y) and \
            Wave_down[0] > MA.loc[ls_down[0]] and \
            MA.loc[ls_down[1]] > Wave_down[1] and \
            (np.abs(pricet - MA[-1]) < 0.6):
        signal = -1
...
with closing(api):
    while True:
        ...
        if api.is_changing(klines[-1], "datetime"):
            ...
            dict_results = wave(klines, w=4, X=20)
...
#
    ...
    end = ys.index.get_loc('2019-04-11 09:57:00')
    ys = ys[end-100:end+1]
